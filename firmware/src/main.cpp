...
void loop()
{
  ...
  // Serial.printf("data: %i tower: %i %i\n", robot_state, tower_left, tower_right);
  ...
  // Serial.printf("euler x: %f euler_new: %f position: %u\n", e.x(), euler.x(), position);
  ...
  else if (state == 1)
  {
    ...
    {
      motor1_controller.spin(-700);
      motor2_controller.spin(700);
      motor3_controller.spin(700);
      delay(750);
      robot_state = 2;
    }
